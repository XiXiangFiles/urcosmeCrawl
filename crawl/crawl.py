...
def crawl(start,end,categoryNumI,serialNumI):
    brandset=Set()
    categoryset=Set()
    productset=Set()
    serialset=Set()
    for brandNumI in range(start,end):
        ...
        for pageNumI in range(1,20):
            ...
            for category in allcategory :
                if categoryNumI ==0:
                    categoryArr.add(category.get('value'))
            ...
            for serial in allserial:
                if serialNumI ==0 :
                    serialArr.add(serial.get('value'))
            ...
            for category in categoryArr :
                for serialnum in serialArr:
                    if(category != '0' and serialnum != '0'):
                        print 'into'
                        crawl(start,start+1,int(category),int(serialnum))
...
def main():
    t = thread.Thread(target = crawl(0,100,0,0))
    t.start()
